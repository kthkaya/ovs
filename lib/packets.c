...
push_th(struct dp_packet *b, ovs_be16 nextUID)
{
	...
	VLOG_INFO("packets.c push_th():Current L4 size %d", prev_l4_size);
	 */

	//struct ip6_hdr *nh = dp_packet_l3(b);
	/*
	struct ovs_16aligned_ip6_hdr *nh = dp_packet_l3(b);
	VLOG_INFO("packets.c push_th():IPv6 Next Header before: %4x",nh->ip6_nxt);
	 */
	//nh->ip6_plen = htons(prev_l4_size + 8);

	//VLOG_INFO("packets.c push_th():Size of Frag header %d", sizeof *fragHdr);
	//fragHdr->ip6f_nxt = nh->ip6_ctlun.ip6_un1.ip6_un1_nxt;
	//nh->ip6_ctlun.ip6_un1.ip6_un1_nxt = 44;
	//nh->ip6_nxt = htons(44);
	/*
	memcpy(&nh->ip6_nxt,&nextHdr,1);
	VLOG_INFO("packets.c push_th():IPv6 Next Header after: %4x",nh->ip6_nxt);
	 */

	/*
	fragHdr->ip6f_ident = 5;
	fragHdr->ip6f_offlg = 6;
	 */
	...
	//dp_packet_adjust_layer_offset(&b->l4_ofs, sizeof *fragHdr);
	...
	struct eth_header *eh = dp_packet_eth(b);
	    ...
	    if (eth_type_vlan(eh->eth_type)) {
	        ovs_be16 *p;
	        char *l2_5 = dp_packet_l2_5(b);

	        p = ALIGNED_CAST(ovs_be16 *,
	                         (l2_5 ? l2_5 : (char *)dp_packet_l3(b)) - 2);
	 *p = ETH_TYPE_IPV6;
	    } else {
	        eh->eth_type = htons(ETH_TYPE_IPV6);
	    }
	...
	//eth->eth_type = ETH_TYPE_IPV6;
	//eth->eth_src.ea[0] = 0xDE;
	//printf("pushhhh_th(): Last 2 of mac %x:%x", eth->eth_dst.ea[4],eth->eth_dst.ea[5]);
		...
		const ovs_be128 prefix)
{
	...
	struct ovs_nd_prefix_opt *prefix_opt = dp_packet_put_uninit(b, sizeof *prefix_opt);
