...
static inline bool
parse_ipv6_ext_hdrs__(struct mf_ctx *mf, const void **datap, size_t *sizep,
					  uint8_t *nw_proto, uint8_t *nw_frag)
{
    while (1) {
        ...
        if ((*nw_proto == IPPROTO_HOPOPTS)
            ...
            *nw_proto = frag_hdr->ip6f_nxt;
        	...
        	const struct ip6_trhdr *trh = (const struct ip6_trhdr *)
        	        					data_try_pull(datap, sizep, TRH_STATIC_LEN);
        	*nw_proto = trh->ip6trh_nxt;
        	miniflow_push_be32((*mf), ip6trh_nextuid, trh->ip6trh_nextuid_flags);
        	return true;
        }
    }
}
